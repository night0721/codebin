...
app.post("/create", createAccountLimiter, async (req, res) => {
  ...
  const d = await ShortUrl.create({
    name: shortid.generate(),
    code: filter.clean(req.body.code),
    title: filter.clean(tit),
    description: req.body.description,
  });
  res.redirect(`/${d.name}`);
});
app.post("/botcreate", createAccountLimiter, async (req, res) => {
  const existing = await ShortUrl.findOne({ code: req.body.code });
  if (existing)
    return res.send({ name: `https://code.cath.gq/${existing.name}` });
  let tit = req.body.title;
  if (tit.length > 25) {
    tit = tit.slice(0, 50) + "...";
  }
  ...
  res.send({ name: `https://code.cath.gq/${d.name}` });
});
app.get("/:shortUrl", async (req, res) => {
  ...
  res.render("index", {
    ...
    showHeader: true,
    views: views,
    title: shortUrl.title ? shortUrl.title : "Code Bin",
